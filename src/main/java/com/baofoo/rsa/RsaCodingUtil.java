...
import com.baofoo.util.StringUtils;
...
public final class RsaCodingUtil {
    ...
    public static String encryptByPublicKey(String src, PublicKey publicKey) {
        ...
        return StringUtils.byte2Hex(destBytes);
    ...
    public static String decryptByPrivateKey(String src, PrivateKey privateKey) {
        ...
        try {
            byte[] destBytes = rsaByPrivateKey(StringUtils.hex2Bytes(src), privateKey, Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
...
//			//log.error("解密内容异常", e);
    ...
    public static String decryptByPublicKey(String src, PublicKey publicKey) {
        ...
        try {
            byte[] destBytes = rsaByPublicKey(StringUtils.hex2Bytes(src), publicKey, Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
